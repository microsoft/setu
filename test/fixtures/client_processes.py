...
def run_polling_dest_client(
    ...
    """Dest client process: submit pull, poll for completion (no blocking wait).
    ...
    try:
        print(f"[PollingDest {client_id}] Waiting for sources...", flush=True)
        ...
        assert not pending, f"Timed out waiting for completions: {pending}"
        print(f"[PollingDest {client_id}] Copy complete via polling!", flush=True)
        ...
        print(
            f"[PollingDest {client_id}] expected={expected_value}, "
        ...
        print(f"[PollingDest {client_id}] ERROR: {e}", flush=True)
        result_queue.put(
            {
                "success": False,
                "client_id": client_id,
