...
void InitTorchTensorIPCPybindClass(py::module_& m) {
  py::class_<TensorIPCSpec, TensorIPCSpecPtr>(m, "TensorIPCSpec")
      .def(py::init<torch::IntArrayRef, torch::IntArrayRef, std::int64_t,
           ...
           py::arg("storage_size_bytes"), py::arg("storage_offset_bytes"),
           py::arg("ref_counter_handle"), py::arg("ref_counter_offset"),
           py::arg("event_handle"), py::arg("event_sync_required"))
  ...
  m.def("prepare_tensor_ipc_spec", &PrepareTensorIPCSpec, py::arg("tensor"),
        "Prepare a tensor for IPC sharing. Returns a TensorIPCSpec containing "
        "all necessary information to reconstruct the tensor in another "
        "process.");
}
//==============================================================================
