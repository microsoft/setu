...
struct TensorSelection {
   ...
   */
  [[nodiscard]] TensorSelectionPtr Select(
   ...
   */
  [[nodiscard]] TensorSelectionPtr Select(const TensorDimName& dim_name,
 ...
 private:
  static TensorIndicesMap BuildIndicesFromDims(TensorDimMap dims_param) {
    ...
    for (const auto& [dim_name, dim] : *dims_param) {
      result_indices[dim_name] = TensorIndicesBitset(dim.size);
    }
