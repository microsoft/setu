...
struct GetTensorSpecRequest : public BaseRequest {
  /// @brief Constructs a request with auto-generated request ID.
  explicit GetTensorSpecRequest(TensorName tensor_name_param)
  ...
  /// deserialization).
  GetTensorSpecRequest(RequestId request_id_param, TensorName tensor_name_param)
  ...
  [[nodiscard]] std::string ToString() const {
    return std::format("GetTensorSpecRequest(request_id={}, tensor_name={})",
                       request_id, tensor_name);
  }

  void Serialize(BinaryBuffer& buffer) const;

  static GetTensorSpecRequest Deserialize(const BinaryRange& range);

  const TensorName tensor_name;
};
using GetTensorSpecRequestPtr = std::shared_ptr<GetTensorSpecRequest>;
